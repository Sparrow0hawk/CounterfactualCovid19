...
class LoadDailyCasesTask {
  getCounterfactualCovidCases = async (
    ...
    try {
      var target = `http://backend:8000/api/cases/counterfactual/daily/normalised/?iso_code=${iso_code}&start_date=${start_date}&end_date=${end_date}`;
  ...
  getRealCovidCases = async (iso_code, start_date, end_date) => {
    try {
      const target = `http://backend:8000/api/cases/real/daily/normalised/?iso_code=${iso_code}&start_date=${start_date}&end_date=${end_date}`;
      console.debug(`Backend ${target}`);
      const response = await axios.get(target, {});
      return response.data;
    } catch (error) {
